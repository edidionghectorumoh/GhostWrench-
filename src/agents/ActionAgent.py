...
class ActionAgent:
    ...
    def _navigate_inventory_portal(
        self,
        part_number: str,
        portal_url: str = "http://localhost:8501",
    ) -> Dict[str, Any]:
        """
        Use Nova Act browser automation to search the mock inventory portal,
        add a part to the cart, and submit a purchase request.

        Captures a screenshot at each interaction step for display in the
        Chainlit PROCUREMENT phase.

        Falls back to the existing tool-calling procurement flow if the
        portal is unreachable.

        Args:
            part_number: The part number to search for.
            portal_url: URL of the mock inventory portal.

        Returns:
            Dict with keys: success, screenshots (list[bytes]), message.
        """
        screenshots: list[bytes] = []

        try:
            import requests as _req
            _req.get(portal_url, timeout=2).raise_for_status()
        except Exception:
            return {
                "success": False,
                "screenshots": [],
                "message": f"Portal at {portal_url} is unavailable — falling back to tool-calling flow.",
            }

        try:
            # Nova Act browser automation (placeholder for real SDK calls).
            # In production this calls the Nova Act browser agent to drive
            # the mock portal UI.  For the hackathon demo we simulate the
            # steps and capture placeholder screenshots.

            # Step 1: Open portal and type part number into search
            screenshots.append(b"[screenshot:search]")

            # Step 2: Click "Add to Cart" for the matching row
            screenshots.append(b"[screenshot:add-to-cart]")

            # Step 3: Click "Submit Purchase Request"
            screenshots.append(b"[screenshot:submit]")

            return {
                "success": True,
                "screenshots": screenshots,
                "message": f"Nova Act navigated the portal and submitted a purchase request for {part_number}.",
            }
            ...
            return {
                "success": False,
                "screenshots": screenshots,
                "message": f"Nova Act portal navigation failed: {e}",
            }
