...
Uses Weaviate vector database with Amazon Nova Multimodal Embeddings for
unified text and image embeddings in a shared 1024-dim semantic space.
"""
...
from config import bedrock_runtime, NOVA_MULTIMODAL_EMBEDDINGS_MODEL_ID, EMBEDDING_DIMENSION
...
class RAGSystem:
    ...
    def __init__(
        ...
        self.embedding_model_id = NOVA_MULTIMODAL_EMBEDDINGS_MODEL_ID
    ...
    def _generate_text_embedding(self, text: str) -> List[float]:
        ...
        Uses the unified multimodal embedding space so text and image
        vectors are directly comparable via cosine similarity.
    ...
    def _generate_image_embedding(self, image_data: bytes) -> List[float]:
        ...
        Uses the same unified embedding space as text, enabling
        cross-modal retrieval (text query → image results and vice versa).
        
        Args:
            image_data: Image binary data (JPEG, PNG, BMP, or TIFF)
