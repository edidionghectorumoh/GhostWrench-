...
class OrchestrationLayer:
    ...
    def _initialize_workflow(
        ...
        workflow_state = WorkflowState(
            ...
            site_id=request.site_context.site_id if request.site_context else None
        )
    ...
    def _handle_intake(
        ...
        if workflow_state.can_transition_to(WorkflowPhase.DIAGNOSIS):
            ...
            return FieldResponse(
                ...
                data={
                    "site_id": request.site_context.site_id,
                    "technician_id": request.technician_id
                },
    ...
    def _handle_diagnosis(
        ...
        self.thought_logger.log_thought(
            ...
            metadata={"site_id": request.site_context.site_id if request.site_context else None}
        )
    ...
    def route_to_diagnosis_agent(self, request: FieldRequest) -> Dict[str, Any]:
        """Route request to diagnosis agent."""
        logger.info("Routing to Diagnosis Agent")
        return self.diagnosis_agent.diagnose_issue(request)
    
    def route_to_action_agent(self, request: FieldRequest) -> Dict[str, Any]:
        """Route request to action agent."""
        logger.info("Routing to Action Agent")
        return self.action_agent.execute_procurement(request)
    
    def route_to_guidance_agent(self, request: FieldRequest) -> Dict[str, Any]:
        """Route request to guidance agent."""
        logger.info("Routing to Guidance Agent")
        return self.guidance_agent.generate_guidance(request)
    ...
    def _create_maintenance_record(
        ...
        record = self.maintenance_client.create_record(
            ...
            component_id=request.site_context.component_id if request.site_context else "unknown",
